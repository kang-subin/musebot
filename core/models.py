...
class DateCalculationResponse(BaseModel):
    response_text: str = Field(description="사용자에게 전달할 날짜 계산 결과 (예: '5일 남았습니다.', '오늘입니다!', '3일 지났습니다.').")
    detected_intent: str = Field(description="항상 'date_calculation'으로 설정.")
    confidence_score: float = Field(description="날짜 계산 결과의 신뢰도 (0.0~1.0).")

class TimeConversionResponse(BaseModel):
    response_text: str = Field(description="사용자에게 전달할 현재 시각 정보 (예: '서울 현재 시각: 2025-08-01 14:35:22').")
    detected_intent: str = Field(description="항상 'time_conversion'으로 설정.")
    confidence_score: float = Field(description="시간 변환 결과의 신뢰도 (0.0~1.0).")
